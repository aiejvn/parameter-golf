...
class MedusaHead(nn.Module):
    # Single linear head predicting a future token offset > 1.
    # Trained jointly with the backbone; stripped from the state dict before export.
    def __init__(self, dim: int, vocab_size: int):
        super().__init__()
        self.linear = CastedLinear(dim, vocab_size, bias=False)
        self.linear._zero_init = True
...
def main() -> None:
    ...
    if len(base_model.medusa_heads) > 0:
        medusa_lr = args.matrix_lr * args.medusa_lr_mult
        optimizer_medusa = Muon(
            [p for p in base_model.medusa_heads.parameters() if p.ndim == 2],
            lr=medusa_lr,
            momentum=args.muon_momentum,
            backend_steps=args.muon_backend_steps,
        )
...
# SWEEP RUNNER
# -----------------------------
# Run with: python sweep_gpt.py --sweep
# Each combination is launched as a subprocess so torch.compile caches and CUDA
