...
class Hyperparameters:
    ...
    # --- KITCHEN SINK: FiLM Conditioning (input-conditioned affine scale/shift per sublayer) ---
    # γ(x)·F + β(x) applied to each attention and MLP output, conditioned on current token state.
    # A rank-r bottleneck (dim→r→4·dim) predicts separate (γ,β) for attn and MLP.
    # Zero-init on the up-projection → identity at step 0; attn_scales[i]/mlp_scales[i] preserved as backbone.
    film_enabled = bool(int(os.environ.get("FILM_ENABLED", "0")))
    film_rank = int(os.environ.get("FILM_RANK", "1"))

    # --- KITCHEN SINK: ACT Halting (adaptive computation time, Graves 2016) ---
    # Learned scalar p_halt(t)=σ(W·x+b) allocates a probability budget across iterations.
    # Output = weighted sum of intermediate states; ponder cost τ·R penalises excess computation.
    # No dynamic control flow — all num_iters always execute (torch.compile compatible).
    act_enabled = bool(int(os.environ.get("ACT_ENABLED", "0")))
    act_ponder_cost = float(os.environ.get("ACT_PONDER_COST", "0.01"))

    # --- KITCHEN SINK: Masked Recurrence (per-token iteration gate) ---
    # gate(t)=σ(W·x+b) interpolates between applying and skipping the block update per token.
    # gate→1: full update; gate→0: hidden state unchanged for this iteration.
    # Composes with ACT: recurrence controls state updates, ACT controls output weighting.
    masked_recur_enabled = bool(int(os.environ.get("MASKED_RECUR_ENABLED", "0")))
...
# -----------------------------
# KITCHEN SINK: FiLM Conditioning
# -----------------------------

class FiLMProjector(nn.Module):
    """Rank-r input-conditioned affine modulator: predicts (γ_a, β_a, γ_m, β_m) per token.

    Applied as γ(x)·F + β(x) to the attention and MLP sublayer outputs separately.
    A rank-r bottleneck (dim→r→4·dim) keeps parameter cost at r·5·dim.
    Zero-init on up_proj → all outputs zero at step 0 → identity start (no change from baseline).
    """
    def __init__(self, dim: int, rank: int = 1):
        super().__init__()
        self.down_proj = CastedLinear(dim, rank, bias=False)
        self.up_proj = CastedLinear(rank, dim * 4, bias=False)
        nn.init.zeros_(self.up_proj.weight)

    def forward(self, x: Tensor) -> tuple[Tensor, Tensor, Tensor, Tensor]:
        # x: (B, T, dim) → h: (B, T, rank) → out: (B, T, dim*4)
        h = self.down_proj(x)
        out = self.up_proj(h)
        gamma_attn, beta_attn, gamma_mlp, beta_mlp = out.chunk(4, dim=-1)
        return gamma_attn, beta_attn, gamma_mlp, beta_mlp


# -----------------------------
# KITCHEN SINK: ACT Halting
# -----------------------------

class ACTHalting(nn.Module):
    """Per-token halt probability for Adaptive Computation Time (Graves 2016).

    p_halt(t) = σ(W·x + b) — a learned opaque proxy for 'am I done?'.
    The model learns to correlate this projection with convergence signals
    (stabilised hidden state, peaky logit distribution) as emergent behaviour.
    Bias initialised to -1.0 → σ(-1) ≈ 0.27 at init (moderate early halting).
    """
    def __init__(self, dim: int):
        super().__init__()
        self.halt_proj = CastedLinear(dim, 1, bias=True)
        nn.init.zeros_(self.halt_proj.weight)
        self.halt_proj.bias.data.fill_(-1.0)  # type: ignore[union-attr]
...
# -----------------------------
# KITCHEN SINK: Masked Recurrence
# -----------------------------

class RecurrenceGate(nn.Module):
    """Per-token gate that interpolates between applying or skipping a block update.

    gate(t) = σ(W·x + b) — same learned-proxy nature as ACTHalting, not explicit entropy.
    gate→1: full block update applied; gate→0: hidden state passes through unchanged.
    Bias initialised to 2.0 → σ(2) ≈ 0.88 at init (near full-update, close to baseline).
    """
    def __init__(self, dim: int):
        super().__init__()
        self.gate_proj = CastedLinear(dim, 1, bias=True)
        nn.init.zeros_(self.gate_proj.weight)
        self.gate_proj.bias.data.fill_(2.0)  # type: ignore[union-attr]
...
class GPT(nn.Module):
    def __init__(
        ...
        masked_recur_enabled: bool = False,
    ):
        super().__init__()
        ...
        # Kitchen sink: FiLM / ACT / masked recurrence (UT-aware)
        self.film_proj = FiLMProjector(model_dim, film_rank) if film_enabled else None
        self.act_halting = ACTHalting(model_dim) if act_enabled else None
        self.recur_gate = RecurrenceGate(model_dim) if masked_recur_enabled else None
        self.act_ponder_cost_val = act_ponder_cost
    ...
    def _shared_block_forward(self, x: Tensor, x0: Tensor, iter_idx: int) -> Tensor:
        ...
        if self.film_proj is not None:
            # FiLM affine: γ(x)·F + β(x) applied separately to attn and MLP outputs.
            # γ_a=0, β_a=0 at init (up_proj zero-init) → (1+0)*F+0 = F, identity start.
            ga, ba, gm, bm = self.film_proj(x)
            attn_out = block.attn(block.attn_norm(x) * s)
            x = x + ip.attn_scales[iter_idx].to(dtype=x.dtype) * (1.0 + ga) * attn_out + ba
            mlp_out = block.mlp(block.mlp_norm(x) * s)
            x = x + ip.mlp_scales[iter_idx].to(dtype=x.dtype) * (1.0 + gm) * mlp_out + bm
        else:
            attn_out = block.attn(block.attn_norm(x) * s)
    ...
    def _apply_recur_gate(self, x: Tensor, x_new: Tensor) -> Tensor:
        """Masked recurrence: soft interpolation between applying or skipping a block update.

        gate→1: full update (x_new); gate→0: hidden state unchanged (x).
        The block is always computed regardless of gate value — no FLOP savings,
        the benefit is in learned representation routing.
        """
        if self.recur_gate is not None:
            gate = self.recur_gate(x)  # (B, T, 1)
            return gate * x_new + (1.0 - gate) * x
        return x_new

    def _run_blocks(self, x: Tensor, x0: Tensor) -> tuple[Tensor, Tensor]:
        """Run encoder/decoder blocks (works for both UT and standard mode).

        Returns (x, ponder_cost) where ponder_cost is the ACT regularisation term
        (mean unspent probability mass). Zero when ACT is disabled.
        """
        ...
        if self.universal_transformer:
            if self.act_halting is not None:
                ...
                for i in range(self.num_encoder_iters):
                    x_new = self._shared_block_forward(x, x0, i)
                    x = self._apply_recur_gate(x, x_new)
                    skips.append(x)
            ...
            # Standard (non-UT) path + optional masked recurrence.
            skips_std: list[Tensor] = []
            for i in range(self.num_encoder_layers):
                x_new = self.blocks[i](x, x0)
                x = self._apply_recur_gate(x, x_new)
                skips_std.append(x)
            for i in range(self.num_decoder_layers):
                if skips_std:
                    ...
                    scaled_skip = self.skip_weights[i].to(dtype=x.dtype)[None, None, :] * skip
                    x = skip_gate[None, None, :] * x + (1.0 - skip_gate[None, None, :]) * scaled_skip
                x_new = self.blocks[self.num_encoder_layers + i](x, x0)
                x = self._apply_recur_gate(x, x_new)
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        ...
        x = self.smear_gate(x)
        x0 = x
        x, ponder_cost = self._run_blocks(x, x0)
        ...
        if self.act_halting is not None:
            loss = loss + self.act_ponder_cost_val * ponder_cost
        return loss
    ...
    def forward_logits(self, input_ids: Tensor) -> Tensor:
        ...
        x = self.smear_gate(x)
        x0 = x
        x, _ponder_cost = self._run_blocks(x, x0)
...
def main() -> None:
    ...
    base_model = GPT(
        ...
        act_ponder_cost=args.act_ponder_cost,
        masked_recur_enabled=args.masked_recur_enabled,
    ).to(device).bfloat16()
    ...
    # Route FiLM / ACT / masked recurrence params: 2D weights → Muon, biases → scalar AdamW
    for attr in ("film_proj", "act_halting", "recur_gate"):
        mod = getattr(base_model, attr, None)
        if mod is not None:
            for name, p in mod.named_parameters():
                if p.ndim == 2 and not any(pat in name for pat in CONTROL_TENSOR_NAME_PATTERNS):
                    matrix_params_enc.append(p)
                else:
                    other_scalar_params.append(p)
    ...
    log0(f"kitchen_sink: UT={args.universal_transformer} num_iters={args.num_iters} echo={args.echo_enabled} quilt={args.quilt_enabled} sparse={args.sparse_enabled} film={args.film_enabled}(rank={args.film_rank}) act={args.act_enabled}(ponder_cost={args.act_ponder_cost}) masked_recur={args.masked_recur_enabled}")
